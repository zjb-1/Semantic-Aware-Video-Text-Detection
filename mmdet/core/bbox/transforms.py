...
def bbox2roi_global(bbox_list, x):
    "convert a list of bboxes to global bboxes"
    rois_list = []
    height = x[0].shape[2] * 4
    width = x[0].shape[3] * 4
    for img_id, bboxes in enumerate(bbox_list):
        if bboxes.size(0) > 0:
            img_inds = bboxes.new_full((bboxes.size(0), 1), img_id)
            temp_bboxes = bboxes.clone()
            temp_bboxes[:, 0] = 1
            temp_bboxes[:, 1] = 1
            temp_bboxes[:, 2] = height
            temp_bboxes[:, 3] = width
            rois = torch.cat([img_inds, temp_bboxes[:, :4]], dim=-1)
        else:
            rois = bboxes.new_zeros((0, 5))
        rois_list.append(rois)
    rois = torch.cat(rois_list, 0)
