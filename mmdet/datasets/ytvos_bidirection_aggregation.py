...
class YTVOSBiDirectionAggregationDataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        for i in range(seq_len):
            if cur_img_idx == min(vid_img_idx) :
                agg_idx = cur_img_idx
            else:
        ...
        # agg imgs after
        for i in range(seq_len):
            if cur_img_idx == max(vid_img_idx) :
                agg_idx = cur_img_idx
            else:
                agg_idx = cur_img_idx + 1 + i
            agg_path = '%s/%05d' % (prefix, agg_idx)
            agg_imgs_after.append(mmcv.imread(osp.join(self.img_prefix, agg_path + '.jpg')))
        # ref image
        ...
        agg_imgs_after_data= []
        for agg_transform in agg_imgs_after_transform:
            agg_data = DC(to_tensor(agg_transform), stack=True)
            agg_imgs_after_data.append(agg_data)
        data = dict(
    ...
    def prepare_test_img(self, idx):
	    ...
	    # agg_img_after
            ...
            for agg_img_after in agg_imgs_after:
                ...
                agg_img_after = agg_img_after.copy()
                agg_imgs_after_transform.append(agg_img_after)
            imgs_after.append(agg_imgs_after_transform)
        data = dict(img=imgs, img_meta=img_metas, agg_before_img=imgs_before,
		agg_after_img=imgs_after)
        return data
