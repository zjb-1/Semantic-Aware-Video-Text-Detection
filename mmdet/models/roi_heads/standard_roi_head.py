...
class StandardRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        losses = dict()
        # bbox head forward and loss & track head and loss
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(x, sampling_results,
                                                    x_feat, ref_x,
                                                    gt_bboxes, gt_labels,
                                                    img_metas, ref_bboxes)
            losses.update(bbox_results['loss_bbox'])
            losses.update(bbox_results['loss_match'])
    ...
    def _bbox_forward_train(self, x, sampling_results, x_feat, ref_x, gt_bboxes, gt_labels,
                            img_metas, ref_bboxes):
        ...
        bbox_track_feats = self.track_roi_extractor(
            x_feat[:self.track_roi_extractor.num_inputs], rois)
        ...
        match_score = self.track_head(bbox_track_feats, ref_bbox_feats,
                                      ...
                                      bbox_sem_feats_input, ref_sem_feats_input)
        loss_match = self.track_head.loss(match_score, ids, id_weights)
        bbox_results.update(loss_match=loss_match)
