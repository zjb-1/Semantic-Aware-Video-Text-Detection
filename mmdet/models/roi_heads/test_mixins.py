...
from mmdet.core import (bbox2roi, bbox_mapping, merge_aug_bboxes,
                        merge_aug_masks, multiclass_nms, merge_aug_proposals)
...
class MaskTestMixin(object):
    ...
    def simple_test_mask(self,
        ...
        if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
            segm_results = [[[] for _ in range(self.roi_head.mask_head.num_classes)]
            ...
            if torch.onnx.is_in_onnx_export():
                ...
                for i, boxes in enumerate(det_bboxes):
                    ...
                    mask_result = self.roi_head._mask_forward(x, mask_rois)
            ...
            for i in range(num_imgs):
                if det_bboxes[i].shape[0] == 0:
                    ...
                    r_bboxes = torch.cat((_bboxes[i], det_bboxes[i][:, -1].unsqueeze(1)), 1) # add the score
                    segm_result = self.roi_head.mask_head.get_seg_masks(
                        mask_preds[i], r_bboxes, det_labels[i],
                        self.test_cfg.rcnn, ori_shapes[i], scale_factors[i],
                        rescale, det_obj_ids=det_obj_ids[i])
    ...
    def aug_test_mask(self, feats, img_metas, det_bboxes, det_labels):
        ...
        if det_bboxes.shape[0] == 0:
            segm_result = [[] for _ in range(self.roi_head.mask_head.num_classes)]
        else:
            aug_masks = []
            for x, img_meta in zip(feats, img_metas):
                img_shape = img_meta[0]['img_shape']
